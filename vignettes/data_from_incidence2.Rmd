...
The default value of this argument is `FALSE`, however, keeping `NA`s in the data causes downstream issues with functions such as `cfr_static()`.
...
We obtain and use a delay distribution from @barry2018 for Ebola virus disease; this is a Gamma distribution with shape $k$ = 2.40 and scale $\theta$ = 3.33.

Finally, we estimate the static case fatality risk while correcting for delays using the `cfr_static()` function.
...
Such usage is shown in other vignettes such as the [vignette on estimating time-varying severity](cfr_time_varying_severity.html).
...
Note that the grouping structure of this dataset given by the "region" variable is present in the `<incidence2>` object, but in this example it is overridden to provide a single dataset with cases and deaths summed across regions by date by `prepare_data()`.
