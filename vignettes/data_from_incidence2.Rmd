...
## Creating `<incidence2>` objects from linelists
...
In this example, we used the default value of the `fill_NA` argument which is `TRUE`; this replaces missing case and death counts in the data with `0`s.
Keeping `NA`s in the data causes downstream issues with functions such as `cfr_static()`.
...
## Creating `<incidence2>` objects from aggregated case data
