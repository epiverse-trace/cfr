...
However, we have named the function `estimate_ascertainment()` as we expect that users will be most interested in situations where testing capacity is itself the limiting factor in ascertainment.
...
We load data provided with the _cfr_ package and initially made available using the _covidregionaldata_ package [@palmer2021].
...
We then subset the data to focus on just the first few months of the outbreak in the United Kingdom.
...
We use the `estimate_ascertainment()` function within the _cfr_ package to calculate the time-varying CFR for the COVID-19 epidemic in the U.K.
...
This known disease severity is passed to the `severity_baseline` argument in `estimate_ascertainment()`, and forms the denominator in the resulting under-ascertainment calculation.
...
For illustrative purposes, we define a large outbreak as one which has caused at least 100,000 deaths between 2020 and 2023, and focus on the period between the start of each outbreak to the 1st of June 2020.

We do so as under-ascertainment of cases and infections was likely to be high in this early period of the pandemic, because testing of symptomatic cases was still being scale up and had not been made widely available in many countries.
...
covid_data_early <- filter(covid_data, date < "2020-06-01")
...
df_reporting %>%
  ...
  geom_pointrange(
    aes(
      x = fct_reorder(country, ascertainment_me),
      y = ascertainment_me,
      ymin = ascertainment_lo,
      ymax = ascertainment_hi
    )
  ) +
  coord_flip() +
