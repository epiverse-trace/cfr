...
The `cfr_time_varying()` function requires a `data.frame` of input data --- typically case and death time series data for estimating a CFR --- and, optionally, a delay distribution density function to allow for delay correction.
...
We retrieve the appropriate distribution reported in @linton2020; this is a lognormal distribution with $\mu$ = 2.577 and $\sigma$ = 0.440.
