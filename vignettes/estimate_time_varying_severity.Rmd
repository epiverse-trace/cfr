...
disease changes over the course of an ongoing outbreak. We wish to estimate one 
of the typical severity quantities used in epidemiology over time: 
the *case fatality risk (CFR)*, *infection hospitality risk (IFR)* or 
*hospitalisation fatality risk (HFR)*. 

We do so using a method that uses a time series of cases, infections of 
hospitalisations (respectively) and deaths with an appropriate delay 
distribution. The methods used here originate from Nishiura et al.
...
* The introduction of non-pharmaceutical interventions (e.g. social distancing,
masks, lockdowns, etc.), reducing the numbers of infections, but not necessarily
the relative number of deaths.

* The development of effective therapeutics, which do not reduce transmission
and hence the numbers of infections, but do reduce the severity of the symptoms 
and therefore the number of hospitalisations and deaths.

* The emergence of variants. Pathogen evolution is highly complex and as such,
variants with a number of different epidemiological characteristics can emerge. 
For example, a more transmissible variant causing more infections, but not a 
higher relative number of hospitalisations or deaths could emerge. However, a
less transmissible, but more severe variant is also possible, resulting in 
fewer infections, but a higher relative number of deaths.
...
within this function follows Nishiura et al. (2009)^[@nishiura2009early]^. 
...
download the data --- using the `covidregionaldata` package --- with the 
following command:
...
```{r message = FALSE, warning = FALSE, eval = TRUE, fig.cap = "\\label{fig:time_varying_cfr_naive} **Figure 3: Example plot of the naive time-varying CFR.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in the U.K., uncorrected for delays."}
plot_time_varying(df_covid_cfr_uk_naive, lower = 0, upper = 5)
```

Lastly, we plot the corrected — for delays — estimate:

```{r message = FALSE, warning = FALSE, eval = TRUE, fig.cap = "\\label{fig:time_varying_cfr_corrected} **Figure 4: Example plot of the corrected time-varying CFR.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in the U.K., corrected for delays."}
plot_time_varying(df_covid_cfr_uk_corrected, lower = 0, upper = 5)
```

# Example using COVID-19 data from multiple countries
...
to the `dplyr` documentation [here](https://dplyr.tidyverse.org/). 
...
df_covid_cfr <- df_covid |>
  ...
  group_map(~estimate_time_varying(
    df_in = .,
    epi_dist = onset_to_death_covid,
    smooth_inputs = TRUE,
    smoothing_window = 7,
...
```{r message = FALSE, warning = FALSE, eval = TRUE, fig.cap = "\\label{fig:time_varying_cfr_corrected} **Figure 5: Example plot of the corrected time-varying CFR.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in Brazil, corrected for delays."}
 subset(df_covid_cfr, country == "Brazil") |>
  plot_time_varying(lower = 0, upper = 5)
```

Next, we plot the time-varying CFR estimate for the COVID-19 outbreak in India:

```{r message = FALSE, warning = FALSE, eval = TRUE, fig.cap = "\\label{fig:time_varying_cfr_corrected} **Figure 6: Example plot of the corrected time-varying CFR.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in Germany deaths in total, corrected for delays."}
 subset(df_covid_cfr, country == "Germany") |>
   plot_time_varying(lower = 0, upper = 5)
```
...
```{r message = FALSE, warning = FALSE, eval = TRUE, fig.cap = "\\label{fig:time_varying_cfr_corrected} **Figure 7: Example plot of the corrected time-varying CFR.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in United States, corrected for delays."}
 subset(df_covid_cfr, country == "United States") |>
   plot_time_varying(lower = 0, upper = 5)
```
...
```{r message = FALSE, warning = FALSE, eval = TRUE, fig.height = 1000/80, fig.cap = "\\label{fig:time_varying_cfr_corrected} **Figure 8: The corrected time-varying CFR for each country with a large outbreak.** We calculate the time-varying CFR for the ongoing COVID-19 epidemic in each country with a large outbreak — defined as any country with more than 100,000 deaths in total — corrected for delays."}
 df_covid_cfr |> 
  ggplot(aes(x = date)) + 
  geom_line(aes(y = severity_me), linetype = "dashed", alpha = 0.25) +
  geom_ribbon(aes(ymin = severity_lo, 
                  ymax = severity_hi),
