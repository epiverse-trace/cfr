...
ggplot() +
  ...
  labs(
    ...
    color = "Outcome type",
    shape = "Event type",
    title = "Onset and outcome dates by individual"
  ) +
  theme_minimal()
```
...
In an extreme scenario where recoveries are not reported, then we effectively have `outcome_recovery()` with an infinite mean, and the above methods will still apply, with the same bias for the filtering approach.