...
As such, we retrieve such a distribution from the literature [@barry2018]; this is a Gamma distribution with shape $k$ = 2.40 and scale $\theta$ = 3.33.

### Estimating incidence of cases (or similar time-series) with a known death outcome

The function `estimate_outcomes()` from the _cfr_ package can estimate the number of cases which have a known death outcome over time.

The resulting data frame contains two new columns, "estimated_outcomes", for the number of known death outcomes expected for each day, and "u_t" for the ratio of cumulative number of estimated known outcomes and the cumulative number of cases reported until each date specified in data.
...
# plot cases and estimated death outcomes as separate layers
ggplot(df_estimated_outcomes_ebola) +
  geom_step(
    aes(
      x = date, y = cases,
      colour = "Cases"
  ...
  geom_step(
    aes(
      x = date, y = estimated_outcomes,
      colour = "Estimated death outcomes"
    )
  ) +
  scale_colour_manual(
    values = c(
      Cases = "steelblue",
      `Estimated death outcomes` = "darkgreen"
    ),
    name = NULL
  ) +
  scale_x_date(
...
Once we estimate the proportion of cases with known death outcomes, we apply the proportion to the number of cases to correct for the delay between onset and death.
...
Users can correct for delays by passing a density function for epidemiological delays to the optional argument `delay_density`.
The delay distribution must be appropriate to the severity measure being calculated.

When no delay density function is provided, a naive severity estimate is calculated, which does not adjust for delays and is simply the total number of deaths in the dataset divided by the total number of cases.
...
We do so, as the CFR changed dramatically as a result of the vaccination campaign that started in December 2020.
...
We use the same method and implementation as in the Ebola example to estimate the number of known death outcomes over time.

```{r}
df_estimated_outcomes_covid <- estimate_outcomes(
...
ggplot(df_estimated_outcomes_covid) +
  geom_step(
    aes(
      x = date, y = cases,
      colour = "Cases"
