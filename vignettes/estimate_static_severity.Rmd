...
This vignette outlines how to use _cfr_ to calculate disease severity
...
_cfr_. Specifically, we will cover
...
This vignette outlines how to use _cfr_ to calculate disease severity in terms of CFR during an ongoing outbreak, using methods from @nishiura2009, using the the core functions within _cfr_.

::: {.alert .alert-info}
## Concept: Correcting for delays in CFR estimation {-}
...
Consider the example below to understand why.
...
outbreak using the _cfr_ package includes:
...
The function within _cfr_ used to estimate the severity of a disease at a
...
The function `known_outcomes()` from the _cfr_ package estimates the 
