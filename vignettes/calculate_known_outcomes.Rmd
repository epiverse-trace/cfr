...
# Overview
This vignette outlines how to use `datadelay` to calculate disease severity during an ongoing outbreak. We wish to estimate either the
*case fatality risk (CFR)* or *hospitalisation fatality risk (HFR)*. We do so using a method that uses a time series of cases and deaths with a
delay distribution. The methods used here originate from Nishiura et al. (2009)^[@nishiura2009early]^.

This vignette demonstrates how to implement the core functions within `datadelay`. Specifically, we will cover

1. Why naive estimates of CFR or HFR generated during an ongoing outbreak require adjusting, given out of sync time series are often being compared.
2. Arrive at estimates over time for the number of symptomatic individuals that will likely be known to have had an outcome $t$ days after
symptom onset
