...
4. Numerically implement the unbiased estimator using maximum likelihood estimation to arrive at point estimates of the CFR or 
HFR, along with a confidence interval.

# Setup: installation and requirements

Installation of `datadelay` requires the package `devtools`. If the user does
not already have this package installed, they can install it with the following
command
...
```{r message = FALSE, warning = FALSE, eval = FALSE}
# installs the datadelay package from Github, as recommended on their
# repository: https://github.com/epiverse-trace/datadelay
install_github(paste0("epiverse-trace", "/", "datadelay"))
```

Additionally, there are two more packages required for the examples in this vignette. The `epiparameter` package is the source of the delay distributions used 
and the `covidregionaldata` package is the source of the COVID-19 case and death time-series used in the second example in this vignette. We install
`epiparameter` using the following command:

```{r, message = FALSE, warning = FALSE, eval = FALSE}
# installs the epiparameter package from Github, as recommended on their
# repository: https://github.com/epiverse-trace/epiparameter
install_github(paste0("epiverse-trace", "/", "epiparameter"))
```
...
Now we load these three packages using the following commands:
...
or hospitalisations-to-date by cases-to-date leads to downward biased estimates of the CFR or HFR (respectively). This is because this calculation does not account 
...
hospitalisation and outcome leads to a consistently downward biased estimate. Hence, adjusting for such delays using an appropriate delay distribution 
is essential for accurate estimates of severity.

# Data required

Calculating a CFR, adjusting for onset-to-death, require three sets of data:

* A time-series of cases;
* A time-series of deaths;
* A delay distribution, describing the probability an individual will die $t$ days after their initial onset of symnptoms. Estimates of such distributions for specific pathogens can obtained from the literature, based on data from previous outbreaks.


# Example with data from the 1976 Ebola outbreak in the Democratic Republic of the Congo

We use data from the 1976 Ebola outbreak in Yambuku to illustrate the methods. First of all, we plot the data from the entire outbreak. Then we subset 
...
We will focus on the roughly the first half of this dataset to illustrate a real-time analysis question. We do so by subsetting the data so that we only include
days before the 18th September 1976, using the following command:
...
For this example, given that we are using Ebola case data and detection of a case is reasonably approximated by symptom onset, we use the distribution 
...
As noted, the method used within this function follows Nishiura et al.^[@nishiura2009early]^. The function calculates a quantity $u_t$ for each day within 
...
It requires a `data.frame` of input data of new cases and new outcomes over time --- such as case and death incidence time series data --- and a delay distribution. The delay distribution we use here comes from the [epiparameter](https://www.github.com/epiverse-trace/epiparameter) package, as described in the previous subsection.
...
between onset-to-death. We do so by using the function `static_cfr()` from the `datadelay` package. This function estimates the proportion of cases with
...
First of all, we subset the data so that we focus on early period of the second wave of COVID-19 in the U.K. We choose this illustrative period
as the CFR changed dramatically as a result of the vaccine campaign. The static severity calculations we are performing 
in this vignette do not account for changes in severity over time, which means there are best suited for short epidemic periods in which severity can be assumed to remain reasonably constant. We download the data --- using
...
df_covid_uk_subset <- subset(
  df_covid_uk,
  date > "2020-07-01" & date <= "2020-11-01"
)
```
...
Using the same functions as the Ebola example, we calculate the naive and corrected CFRs for the COVID-19 epidemic in late 2020 the U.K:
