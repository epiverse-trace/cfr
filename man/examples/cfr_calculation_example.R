...
# Define onset-to-death delay distributions
o2d_mean <- 15; o2d_sd <- 5 # Mean and S.D. of onset-to-death lognormal distribution
rep_val <- ms_to_logms(o2d_mean,o2d_sd) # Convert to lognormal values
onset_to_death <- function(x){plnorm(x,meanlog=rep_val[["logmean"]],sdlog=rep_val[["logsd"]]) - 
    plnorm(x-1,meanlog=rep_val[["logmean"]],sdlog=rep_val[["logsd"]])} # Define distribution

# Estimate CFR over time
cfr_est <- cfr_calculation(case_time,case_data,death_data,onset_to_death)
